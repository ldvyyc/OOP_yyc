...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (node_type == "P") tf.InsertPlaceholder(node_name);//插入占位符
        else if (node_type == "C") {
            float value;
            cin >> value;
            tf.InsertConstant(node_name, value);//插入常数
        }
    }
    ...
    for (int i = 0; i < q; ++i) {        //读入操作信息
        ...
        if (op == "EVAL") {
            ...
            //cout << "hhh" << endl;
            tf.Eval(node_name, argument_num, argument_name_list_, argument_value_list_);//最后计算
